...
Group: Applications/Databases
Requires: postgresql-server%{?_isa} >= 8.4.0
Requires: postgresql-server%{?_isa} < 8.5
...
sed -i	-e 's|/etc/sysconfig/pgsql|%{_sysconfdir}/sysconfig/pgsql|g' \
	-e 's|/usr/bin|%{_bindir}|g' \
	-e 's|=/var/lib/|=%{?_scl_root}/var/lib/|g' \
	-e 's|/var/run/|%{?_scl_root}/var/run/|g' \
	-e 's|/var/lock/|%{?_scl_root}/var/lock/|g' \
	-e 's|^PGVERSION=.*$|PGVERSION=%{version}|' \
	-e 's|^PGENGINE=.*$|PGENGINE=%{_bindir}|' \
	-e 's|^PREVMAJORVERSION=.*$|PREVMAJORVERSION=%{prevmajorversion}|' \
	-e 's|^PREVPGENGINE=.*$|PREVPGENGINE=/usr/bin|' \
	-e 's|SERVICE_NAME=postgresql|SERVICE_NAME=%{?scl_prefix}postgresql|' postgresql.init
...
* Thu Mar 05 2015 Jozef Mlich <jmlich@redhat.com> - 9.2.10-2
- Release bump, because of previous troubles with ppc64
  and s360x arches and scl metapackage.
  Resolves: #1198663
...
* Tue Oct 14 2014 Honza Horak <hhorak@redhat.com> - 9.2.8-2
- Rebuild for s390x
  Resolves: #1152432
...
* Tue Mar 11 2014 Honza Horak <hhorak@redhat.com> - 9.2.7-2
- Rebuild for keep correct upgrade path
  Resolves: #1074603

* Thu Feb 20 2014 Jozef Mlich <jmlich@redhat.com> 9.2.7-1
...
* Thu Feb 13 2014 Jozef Mlich <jmlich@rehat.com> 9.2.6-2
...
* Wed Nov 27 2013 Honza Horak <hhorak@redhat.com> 9.2.5-7
- Use --no-backup-if-mismatch to avoid inclusion patch left-overs

* Fri Nov 22 2013 Honza Horak <hhorak@redhat.com> 9.2.5-6
- Back-port some patches from fedora
- Rename variable to match postgresql meta-package
